...
class wirecard
{
	...
	function AutoRecurringCharge ($client_id, $order_id, $gateway, $params) {		
		return $this->ChargeRecurring($client_id, $gateway, $order_id, $params['api_payment_reference'], $params['amount']);
	}
	
	function ChargeRecurring($client_id, $gateway, $order_id, $GuWID, $amount)
	{		
		$CI =& get_instance();
	...
	function Process($gateway, $transaction_array)
	{
		...
		$response = $CI->arraytoxml->ToArray($post_response);
