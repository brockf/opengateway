...
class Cron extends Controller {
	...
	function SubscriptionMaintenance()
	{
		...
		$subscriptions = $this->subscription_model->GetAllSubscriptionsByChargeDate($today);
		...
		if($charges) {
			foreach($charges as $charge) {
				if($this->email->TriggerTrip('recurring_autorecur_in_week', $charge['client_id'], false, $charge['subscription_id'])) {
		...
		if($charges) {
			...
			foreach($charges as $charge) {
				if($this->email->TriggerTrip('recurring_autorecur_in_month', $charge['client_id'])) {
		...
		if($charges) {
			$this->load->library('email');
			foreach($charges as $charge) {
				if($this->email->TriggerTrip('recurring_expiring_in_week', $charge['client_id'])) {
		...
		if($charges) {
			$this->load->library('email');
			foreach($charges as $charge) {
				if($this->email->TriggerTrip('recurring_expiring_in_month', $charge['client_id'])) {
